  ...
  const { data: propostas = [], isLoading: loadingPropostas } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("id, status, lista_id, proposta:proposta_id (id, nome, numero_proposta, descricao, id_proposta, observacoes)")
            ...
            <Select value={propostaId} onValueChange={setPropostaId} disabled={!campanhaId || loadingPropostas}>
              ...
              <SelectContent>
                {propostas.map((p: any) => {
                  ...
                  const titulo =
                    p.proposta?.nome ||
                    p.proposta?.descricao ||
                    p.proposta?.id_proposta;
                  const label = titulo
                    ? `${num ? `#${num} — ` : ""}${titulo}`
                    : num
                    ? `Proposta #${num}`
                    : p.proposta?.id?.slice(0, 8) || "Proposta";
                  return (
