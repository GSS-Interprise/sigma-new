...
function Calendar({ 
  ...
  const containerClass = "w-[280px] min-h-[300px] p-3 pointer-events-auto text-inherit";
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        cell: "h-9 w-9 text-center text-sm p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent/90 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(buttonVariants({ variant: "ghost" }), "h-9 w-9 p-0 font-normal text-inherit aria-selected:opacity-100"),
