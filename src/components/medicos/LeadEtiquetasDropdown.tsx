import { useState, useRef, useEffect } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Input } from "@/components/ui/input";
import { Trash2, MoreHorizontal, X } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";

const TAG_COLORS = [
  { id: "blue", bg: "bg-blue-600", hover: "hover:bg-blue-700" },
  { id: "teal", bg: "bg-teal-600", hover: "hover:bg-teal-700" },
  { id: "green", bg: "bg-green-600", hover: "hover:bg-green-700" },
  { id: "yellow", bg: "bg-yellow-500", hover: "hover:bg-yellow-600" },
  { id: "orange", bg: "bg-orange-500", hover: "hover:bg-orange-600" },
  { id: "red", bg: "bg-red-500", hover: "hover:bg-red-600" },
  { id: "pink", bg: "bg-pink-500", hover: "hover:bg-pink-600" },
  { id: "purple", bg: "bg-purple-600", hover: "hover:bg-purple-700" },
  { id: "gray", bg: "bg-gray-500", hover: "hover:bg-gray-600" },
  { id: "stone", bg: "bg-stone-400", hover: "hover:bg-stone-500" },
  { id: "black", bg: "bg-black", hover: "hover:bg-gray-900" },
];

interface TagConfig {
  name: string;
  colorId: string;
}

interface LeadEtiquetasDropdownProps {
  leadId: string;
  selectedTags: string[];
  onTagsChange: (tags: string[]) => void;
  disabled?: boolean;
}

export function LeadEtiquetasDropdown({
  leadId,
  selectedTags,
  onTagsChange,
  disabled = false,
}: LeadEtiquetasDropdownProps) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);
  const queryClient = useQueryClient();

  useEffect(() => {
    if (open && inputRef.current) {
      inputRef.current.focus();
    }
  }, [open]);

  // Fetch available tags from config
  const { data: availableTags = [] } = useQuery({
    queryKey: ["leads-etiquetas-config"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("leads_etiquetas_config")
        .select("*")
        .order("nome");
      if (error) throw error;
      return data.map((tag: any) => ({
        name: tag.nome,
        colorId: tag.cor_id,
      })) as TagConfig[];
    },
  });

  // Mutation to add new tag to config
  const addTagMutation = useMutation({
    mutationFn: async ({ nome, cor_id }: { nome: string; cor_id: string }) => {
      const { error } = await supabase
        .from("leads_etiquetas_config")
        .insert({ nome, cor_id });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leads-etiquetas-config"] });
    },
  });

  // Mutation to update tag color
  const updateTagColorMutation = useMutation({
    mutationFn: async ({ nome, cor_id }: { nome: string; cor_id: string }) => {
      const { error } = await supabase
        .from("leads_etiquetas_config")
        .update({ cor_id })
        .eq("nome", nome);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leads-etiquetas-config"] });
    },
  });

  // Mutation to delete tag from config
  const deleteTagMutation = useMutation({
    mutationFn: async (nome: string) => {
      const { error } = await supabase
        .from("leads_etiquetas_config")
        .delete()
        .eq("nome", nome);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["leads-etiquetas-config"] });
    },
  });

  // Mutation to update lead tags
  const updateLeadTagsMutation = useMutation({
    mutationFn: async (tags: string[]) => {
      const { error } = await supabase
        .from("leads")
        .update({ tags })
        .eq("id", leadId);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lead-prontuario", leadId] });
      queryClient.invalidateQueries({ queryKey: ["leads"] });
    },
  });

  const getTagColor = (tagName: string) => {
    const tag = availableTags.find((t) => t.name === tagName);
    const colorId = tag?.colorId || "gray";
    return TAG_COLORS.find((c) => c.id === colorId) || TAG_COLORS[8];
  };

  const filteredTags = availableTags.filter((tag) =>
    tag.name && tag.name.toLowerCase().includes(search.toLowerCase())
  );

  const canAddNew =
    search.trim() &&
    !availableTags.some(
      (t) => t.name && t.name.toLowerCase() === search.trim().toLowerCase()
    );

  const handleToggleTag = (tagName: string) => {
    const newTags = selectedTags.includes(tagName)
      ? selectedTags.filter((t) => t !== tagName)
      : [...selectedTags, tagName];
    onTagsChange(newTags);
    updateLeadTagsMutation.mutate(newTags);
  };

  const handleRemoveTag = (tagName: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const newTags = selectedTags.filter((t) => t !== tagName);
    onTagsChange(newTags);
    updateLeadTagsMutation.mutate(newTags);
  };

  const handleAddNewTag = () => {
    if (canAddNew) {
      addTagMutation.mutate(
        { nome: search.trim(), cor_id: "gray" },
        {
          onSuccess: () => {
            const newTags = [...selectedTags, search.trim()];
            onTagsChange(newTags);
            updateLeadTagsMutation.mutate(newTags);
            setSearch("");
          },
        }
      );
    }
  };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && canAddNew) {
      handleAddNewTag();
    }
  };

  const handleUpdateTagColor = (tagName: string, colorId: string) => {
    updateTagColorMutation.mutate({ nome: tagName, cor_id: colorId });
  };

  const handleDeleteTag = (tagName: string) => {
    // Remove from selected tags first
    if (selectedTags.includes(tagName)) {
      const newTags = selectedTags.filter((t) => t !== tagName);
      onTagsChange(newTags);
      updateLeadTagsMutation.mutate(newTags);
    }
    // Then delete from config
    deleteTagMutation.mutate(tagName);
    toast.success("Etiqueta excluída");
  };

  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      {/* Display selected tags */}
      {selectedTags.map((tag) => {
        const color = getTagColor(tag);
        return (
          <span
            key={tag}
            className={cn(
              "inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium text-white",
              color.bg
            )}
          >
            {tag}
            {!disabled && (
              <button
                onClick={(e) => handleRemoveTag(tag, e)}
                className="hover:bg-white/20 rounded-full p-0.5"
              >
                <X className="h-3 w-3" />
              </button>
            )}
          </span>
        );
      })}

      {/* Add tag button */}
      {!disabled && (
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <button className="text-xs text-muted-foreground hover:text-foreground px-1.5 py-0.5 rounded hover:bg-accent transition-colors">
              + Etiqueta
            </button>
          </PopoverTrigger>
          <PopoverContent
            className="w-[280px] p-0 bg-popover border shadow-lg z-50"
            align="start"
            sideOffset={4}
          >
            <div className="p-2 border-b">
              <Input
                ref={inputRef}
                placeholder="Pesquise ou adicione tags..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                onKeyDown={handleKeyDown}
                className="h-8 text-sm bg-transparent border-0 focus-visible:ring-0 px-1"
              />
            </div>

            <div className="max-h-[300px] overflow-y-auto p-1">
              {filteredTags.map((tag) => {
                const color = getTagColor(tag.name);
                const isSelected = selectedTags.includes(tag.name);

                return (
                  <div
                    key={tag.name}
                    className="flex items-center justify-between group px-1 py-0.5 rounded hover:bg-accent/50"
                  >
                    <button
                      onClick={() => handleToggleTag(tag.name)}
                      className="flex-1 text-left"
                    >
                      <span
                        className={cn(
                          "inline-flex items-center px-2.5 py-1 rounded text-xs font-medium text-white",
                          color.bg,
                          isSelected &&
                            "ring-2 ring-primary ring-offset-1 ring-offset-background"
                        )}
                      >
                        {tag.name}
                      </span>
                    </button>

                    <Popover>
                      <PopoverTrigger asChild>
                        <button
                          className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-accent transition-opacity"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <MoreHorizontal className="h-4 w-4 text-muted-foreground" />
                        </button>
                      </PopoverTrigger>
                      <PopoverContent
                        className="w-[200px] p-3 bg-popover border shadow-lg z-[60]"
                        align="end"
                        side="right"
                      >
                        <div className="space-y-3">
                          <div className="flex flex-wrap gap-2">
                            {TAG_COLORS.map((c) => (
                              <button
                                key={c.id}
                                onClick={() =>
                                  handleUpdateTagColor(tag.name, c.id)
                                }
                                className={cn(
                                  "w-6 h-6 rounded-full transition-transform hover:scale-110",
                                  c.bg,
                                  tag.colorId === c.id &&
                                    "ring-2 ring-primary ring-offset-2 ring-offset-popover"
                                )}
                              />
                            ))}
                          </div>

                          <button
                            onClick={() => handleDeleteTag(tag.name)}
                            className="flex items-center gap-2 w-full text-sm text-muted-foreground hover:text-destructive py-1"
                          >
                            <Trash2 className="h-4 w-4" />
                            Excluir
                          </button>
                        </div>
                      </PopoverContent>
                    </Popover>
                  </div>
                );
              })}

              {canAddNew && (
                <button
                  onClick={handleAddNewTag}
                  className="w-full text-left px-2 py-2 text-sm text-muted-foreground hover:bg-accent/50 rounded"
                >
                  Criar "
                  <span className="font-medium text-foreground">
                    {search.trim()}
                  </span>
                  "
                </button>
              )}

              {filteredTags.length === 0 && !canAddNew && (
                <div className="px-2 py-4 text-sm text-muted-foreground text-center">
                  Nenhuma tag encontrada
                </div>
              )}
            </div>
          </PopoverContent>
        </Popover>
      )}
    </div>
  );
}

export { TAG_COLORS };
export type { TagConfig };
