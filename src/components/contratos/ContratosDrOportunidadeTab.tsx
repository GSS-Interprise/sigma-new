...
const TABLE_CONFIG = {
  contratos: 'contratos_dr_oportunidade',
  contrato_itens: 'contrato_itens_dr_oportunidade',
  contrato_renovacoes: 'contrato_renovacoes_dr_oportunidade',
  contrato_aditivos_tempo: 'contrato_aditivos_tempo_dr_oportunidade',
  contrato_anexos: 'contrato_anexos_dr_oportunidade',
  queryKey: 'contratos-dr-oportunidade',
  storageBucket: 'contratos-documentos',
};

export function ContratosDrOportunidadeTab() {
  ...
  const { data: contratos, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('contratos_dr_oportunidade')
        .select(`
          *,
          ...
          contrato_anexos_dr_oportunidade(id, arquivo_nome, arquivo_url),
          contrato_aditivos_tempo_dr_oportunidade(id, data_inicio, data_termino, prazo_meses, observacoes)
        `)
      ...
      return data?.map(c => ({
        ...c,
        contrato_anexos: c.contrato_anexos_dr_oportunidade,
        contrato_aditivos_tempo: c.contrato_aditivos_tempo_dr_oportunidade,
      }));
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contratos-dr-oportunidade'] });
  ...
  return (
    <div className="space-y-4">
      ...
      <ContratoDialogWithClient
        ...
        dialogTitle="Contrato Dr. Oportunidade"
        allowCustomTipoContratacao={true}
        tipoContratacaoCampoNome="tipo_contratacao_dr_oportunidade"
      />
    </div>
  );
}
