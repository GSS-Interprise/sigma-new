...
import { Upload, Trash2, ExternalLink, Link2, Loader2, Eye, Download, File, FileText, Image, FileArchive } from "lucide-react";
...
interface AgesContratoDocumentosProps {
  contratoId: string;
}
...
const AgesContratoDocumentos = ({ contratoId }: AgesContratoDocumentosProps) => {
  const { user } = useAuth();
  ...
  const { data: documentos = [], isLoading } = useQuery({
    queryKey: ["ages-contrato-documentos", contratoId],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ages-contrato-documentos", contratoId] });
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      try {
        ...
        const filePath = `contratos/${contratoId}/${Date.now()}_${sanitizedName}`;
...
export default AgesContratoDocumentos;
