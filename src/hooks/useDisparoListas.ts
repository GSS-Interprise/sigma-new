...
export const DISPARO_LISTA_PAGE_SIZE = 50;

export function useDisparoListaItens(
  listaId: string | null,
  page: number = 1,
  pageSize: number = DISPARO_LISTA_PAGE_SIZE,
) {
  return useQuery({
    queryKey: ["disparo-lista-itens", listaId, page, pageSize],
    queryFn: async () => {
      if (!listaId) return { itens: [], total: 0 };
