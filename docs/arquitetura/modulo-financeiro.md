...
- [~] **T06 — Solicitar NF + rastrear resposta.**
  - [x] **T06a (outbound) ✅ 07/07** (`financeiro-solicitar-nf` deployada + botão "Solicitar NF" no Detalhe): envia por email (via `send-email-resend`) com **reply-to tokenizado** `nf+<pagamento_id>@<financeiro_nf_reply_domain>`, opcional WhatsApp, seta `nf_status=solicitada`, loga em `sigma_email_log`. Suporta `email_override` p/ simulação. 752/752 médicos têm email.
  - [ ] **T06b (inbound) — rastreio automático da NF.** Domínio de recebimento no Resend (MX num subdomínio GSS, ex.: `nf.gestaoservicosaude.com.br`) → médico responde com a NF em PDF → webhook `financeiro-nf-inbound` casa pelo token, baixa o anexo (Resend Attachments API), sobe em `financeiro-anexos`, seta `nf_status=recebida` + avisa no canal. **Bloqueio: DNS (MX) no subdomínio — setup Raul/Ewerton.**
- [ ] **T07 — Comprovantes.** Upload do comprovante ao pagar (`tipo=comprovante`) + edge `financeiro-enviar-comprovante` (médico e/ou contabilidade). Deploy.
