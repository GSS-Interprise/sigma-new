...
- [ ] **T05 — Canal "Financeiro" na Comunicação + aprovação dos sócios (mobile).** Cria canal dedicado; ao conferir, publica a produção/pagamento nele; sócio vê valor+NF e aprova pelo **celular** (`pendente→aprovado`, hoje a UI pula direto). Substitui o Slack. Testar Safari iOS + Chrome Android.
- [ ] **T06 — Edge `financeiro-solicitar-nf`.** Ao OK, avisa o médico (WhatsApp/e-mail via `send-whatsapp`/`send-email-resend`) pra emitir NF no valor; `nf_status=solicitada`. Recebimento: upload da NF → `nf_status=recebida`. Deploy.
