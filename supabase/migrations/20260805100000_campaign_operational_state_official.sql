-- Official WhatsApp campaigns do not depend on Evolution chips.
-- Keep the operational state honest for both providers.
...
SELECT
  ...
  CASE
    ...
    WHEN whatsapp_provider = 'twilio'
      AND (official_sender_id IS NULL OR official_template_id IS NULL) THEN 'configurando'
    WHEN whatsapp_provider <> 'twilio' AND chips_configured_calc = 0 THEN 'sem_chip'
    WHEN whatsapp_provider <> 'twilio' AND chips_usable_calc = 0 AND chips_restricted_calc > 0 THEN 'restrita'
    WHEN whatsapp_provider <> 'twilio' AND chips_usable_calc = 0 THEN 'desconectada'
  ...
  CASE
    ...
    WHEN status::text = 'pausada' THEN 'Pausada pela operacao'
    WHEN status::text IN ('planejada', 'rascunho') THEN 'Configuracao ainda nao ativada'
    WHEN whatsapp_provider = 'twilio'
      AND (official_sender_id IS NULL OR official_template_id IS NULL) THEN 'Numero ou template oficial nao configurado'
    WHEN whatsapp_provider <> 'twilio' AND chips_configured_calc = 0 THEN 'Nenhum chip configurado'
    WHEN whatsapp_provider <> 'twilio' AND chips_usable_calc = 0 AND chips_restricted_calc > 0 THEN 'Todos os chips disponiveis estao restritos'
    WHEN whatsapp_provider <> 'twilio' AND chips_usable_calc = 0 THEN 'Nenhum chip operacional conectado'
    ...
    WHEN ultimo_disparo >= now() - interval '30 minutes' THEN 'Envio observado nos ultimos 30 minutos'
    WHEN next_batch_at > now() THEN 'Aguardando a proxima tentativa programada'
