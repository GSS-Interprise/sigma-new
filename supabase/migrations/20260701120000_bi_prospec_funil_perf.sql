-- Perf (01/07): o load da aba Funil & Oportunidades levava ~5.6s porque contava
-- o universo por especialidade juntando leads (813k) × lead_especialidades (541k)
-- mesmo SEM filtro de UF. Otimização: só junta `leads` quando precisa do UF
-- (filtro por estado OU dimensão por estado); no caso default conta direto da junção.
...
-- universo por dimensão: só junta leads quando precisa do UF (filtro OU dimensão estado)
op_universo as (
  select k, count(distinct lead_id) universo from (
    select (case when p_especialidade_id is null then le.especialidade_id::text else l.uf end) k, le.lead_id
    from lead_especialidades le
    left join leads l on l.id = le.lead_id and (p_uf is not null or p_especialidade_id is not null)
    where (p_uf is null or l.uf = p_uf)
...
select json_build_object(
  'funil', (select json_build_object(
     'universo', (select count(distinct le.lead_id) from lead_especialidades le
        left join leads l on l.id = le.lead_id and p_uf is not null
        where (p_uf is null or l.uf = p_uf)
          and (p_especialidade_id is null or le.especialidade_id = p_especialidade_id)),
