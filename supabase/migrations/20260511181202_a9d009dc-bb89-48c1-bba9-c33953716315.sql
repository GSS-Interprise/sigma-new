-- Backfill: copy attachments from licitacoes_anexos table, licitacoes-anexos bucket and editais-pdfs bucket
-- into contrato_rascunho_anexos and contrato_anexos for existing pre-contracts.

-- 1) Backfill contrato_rascunho_anexos for active rascunhos
WITH active_rascunho AS (
  SELECT id AS rascunho_id, licitacao_id
  FROM public.contrato_rascunho
  WHERE licitacao_id IS NOT NULL AND status <> 'cancelado'
), src AS (
  -- from licitacoes_anexos table
  SELECT ar.rascunho_id, ar.licitacao_id,
         la.arquivo_nome,
         la.arquivo_url,
         'licitacoes-anexos/' || la.arquivo_url AS arquivo_path,
         'licitacao_card'::text AS origem
  FROM active_rascunho ar
  JOIN public.licitacoes_anexos la ON la.licitacao_id = ar.licitacao_id
  UNION
  -- from licitacoes-anexos bucket
  SELECT ar.rascunho_id, ar.licitacao_id,
         split_part(o.name, '/', 2) AS arquivo_nome,
         (ar.licitacao_id::text || '/' || split_part(o.name, '/', 2)) AS arquivo_url,
         'licitacoes-anexos/' || o.name AS arquivo_path,
         'licitacao_card'::text AS origem
  FROM active_rascunho ar
  JOIN storage.objects o ON o.bucket_id = 'licitacoes-anexos' AND o.name LIKE ar.licitacao_id::text || '/%'
  UNION
  -- from editais-pdfs bucket
  SELECT ar.rascunho_id, ar.licitacao_id,
         split_part(o.name, '/', 2) AS arquivo_nome,
         ('editais-pdfs/' || ar.licitacao_id::text || '/' || split_part(o.name, '/', 2)) AS arquivo_url,
         'editais-pdfs/' || o.name AS arquivo_path,
         'licitacao_edital'::text AS origem
  FROM active_rascunho ar
  JOIN storage.objects o ON o.bucket_id = 'editais-pdfs' AND o.name LIKE ar.licitacao_id::text || '/%'
)
INSERT INTO public.contrato_rascunho_anexos (contrato_rascunho_id, arquivo_url, arquivo_nome, arquivo_path, origem)
SELECT s.rascunho_id, s.arquivo_url, s.arquivo_nome, s.arquivo_path, s.origem
FROM src s
WHERE NOT EXISTS (
  SELECT 1 FROM public.contrato_rascunho_anexos cra
  WHERE cra.contrato_rascunho_id = s.rascunho_id
    AND (cra.arquivo_path = s.arquivo_path OR cra.arquivo_url = s.arquivo_url OR cra.arquivo_nome = s.arquivo_nome)
);

-- 2) Backfill contrato_anexos for Pre-Contrato rows
WITH pre AS (
  SELECT c.id AS contrato_id, c.licitacao_origem_id AS licitacao_id
  FROM public.contratos c
  WHERE c.status_contrato = 'Pre-Contrato' AND c.licitacao_origem_id IS NOT NULL
), src AS (
  SELECT p.contrato_id,
         la.arquivo_nome,
         'licitacoes-anexos/' || la.arquivo_url AS arquivo_url
  FROM pre p
  JOIN public.licitacoes_anexos la ON la.licitacao_id = p.licitacao_id
  UNION
  SELECT p.contrato_id,
         split_part(o.name, '/', 2) AS arquivo_nome,
         'licitacoes-anexos/' || o.name AS arquivo_url
  FROM pre p
  JOIN storage.objects o ON o.bucket_id = 'licitacoes-anexos' AND o.name LIKE p.licitacao_id::text || '/%'
  UNION
  SELECT p.contrato_id,
         split_part(o.name, '/', 2) AS arquivo_nome,
         'editais-pdfs/' || o.name AS arquivo_url
  FROM pre p
  JOIN storage.objects o ON o.bucket_id = 'editais-pdfs' AND o.name LIKE p.licitacao_id::text || '/%'
)
INSERT INTO public.contrato_anexos (contrato_id, arquivo_nome, arquivo_url, usuario_nome)
SELECT s.contrato_id, s.arquivo_nome, s.arquivo_url, 'Sistema (backfill anexos)'
FROM src s
WHERE NOT EXISTS (
  SELECT 1 FROM public.contrato_anexos ca
  WHERE ca.contrato_id = s.contrato_id
    AND (ca.arquivo_url = s.arquivo_url OR ca.arquivo_nome = s.arquivo_nome)
);

-- 3) Update trigger to also handle attachments from both buckets when a licitacao goes to 'arrematados'
...
BEGIN
  IF NEW.status = 'arrematados' AND (OLD.status IS NULL OR OLD.status != 'arrematados') THEN
    ...
    -- Pre-contrato (sem cliente) - reaproveita ou cria
    ...
    -- Copiar anexos de TODAS as origens para o rascunho (sem duplicatas)
    ...
    -- Copiar anexos de TODAS as origens para o pre-contrato (sem duplicatas)
