...
SELECT 'lead-canal-'::text || cplc.canal AS id,
    ...
    ('/disparos/acompanhamento?canal='::text || cplc.canal) AS link
...
UNION ALL
 SELECT 'contrato-'::text || c.id::text AS id,
    ...
    ('/contratos?open='::text || c.id::text) AS link
   FROM contratos c
     CROSS JOIN LATERAL ( SELECT GREATEST(c.data_fim, ( SELECT max(at.data_termino) AS max
                   FROM contrato_aditivos_tempo at
                  WHERE at.contrato_id = c.id)) AS data_fim_efetiva) eff
  WHERE eff.data_fim_efetiva IS NOT NULL AND eff.data_fim_efetiva <= (CURRENT_DATE + COALESCE(c.dias_aviso_vencimento, 60)::double precision * '1 day'::interval) AND lower(COALESCE(c.status_contrato, ''::text)) = 'ativo'::text
UNION ALL
 SELECT 'medico-doc-'::text || md.id::text AS id,
    ...
    ('/medicos?open='::text || md.medico_id::text || '&doc='::text || md.id::text) AS link
...
UNION ALL
 SELECT 'ages-prof-doc-'::text || apd.id::text AS id,
    ...
    ('/ages?open='::text || apd.profissional_id::text || '&doc='::text || apd.id::text) AS link
...
UNION ALL
 SELECT 'licitacao-'::text || li.id::text AS id,
    ...
    ('/licitacoes?open='::text || li.id::text) AS link
   FROM licitacoes li
  WHERE li.data_limite >= CURRENT_DATE AND li.data_limite <= (CURRENT_DATE + '14 days'::interval) AND (li.status <> ALL (ARRAY['descarte_edital'::status_licitacao, 'suspenso_revogado'::status_licitacao, 'nao_ganhamos'::status_licitacao, 'adjudicacao_homologacao'::status_licitacao, 'arrematados'::status_licitacao]));
