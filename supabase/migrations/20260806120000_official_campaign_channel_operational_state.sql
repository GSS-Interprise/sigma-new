-- Canal oficial: indicadores próprios, sem depender de chips Evolution.
-- A view v2 evita misturar capacidade de portfólio Twilio com saúde de chip.

-- O piloto oficial foi definido em 250 novos contatos/dia. Corrige campanhas
-- oficiais que nasceram com o default antigo de 30; limites maiores/customizados
-- permanecem intactos.
UPDATE public.campanhas
SET limite_diario_campanha = 250
WHERE whatsapp_provider = 'twilio'
  AND coalesce(limite_diario_campanha, 30) <= 30;
...
base AS (
  SELECT
    ...
    ud.ultimo_disparo,
    (c.whatsapp_provider = 'twilio' AND c.official_sender_id IS NOT NULL) AS official_sender_configured,
    (c.whatsapp_provider = 'twilio' AND c.official_template_id IS NOT NULL) AS official_template_configured,
    coalesce(c.limite_diario_campanha, CASE WHEN c.whatsapp_provider = 'twilio' THEN 250 ELSE 30 END) AS limite_diario_efetivo
  FROM public.campanhas c
...
states AS (
  SELECT
    ...
    CASE
      ...
      WHEN whatsapp_provider = 'twilio' AND NOT official_sender_configured THEN 'configurando'
      WHEN whatsapp_provider = 'twilio' AND NOT official_template_configured THEN 'configurando'
      ...
      WHEN whatsapp_provider = 'twilio' THEN 'aguardando'
...
SELECT
  ...
  CASE
    ...
    WHEN operational_state_calc = 'configurando' AND whatsapp_provider = 'twilio' AND NOT official_sender_configured THEN 'API oficial sem número remetente configurado'
    WHEN operational_state_calc = 'configurando' AND whatsapp_provider = 'twilio' THEN 'API oficial sem template aprovado configurado'
    ...
    WHEN operational_state_calc = 'limite_atingido' AND whatsapp_provider = 'twilio' THEN 'Limite diário da API oficial atingido'
    WHEN operational_state_calc = 'limite_atingido' THEN 'Limite diário da campanha atingido'
    WHEN operational_state_calc = 'rodando' AND whatsapp_provider = 'twilio' THEN 'API oficial enviando templates'
    WHEN operational_state_calc = 'rodando' THEN 'Evolution enviando pelos chips conectados'
    WHEN operational_state_calc = 'aguardando' AND whatsapp_provider = 'twilio' THEN 'API oficial ativa; aguardando o próximo lote'
