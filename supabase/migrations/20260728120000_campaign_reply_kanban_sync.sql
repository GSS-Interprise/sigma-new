-- Keep campaign Kanban state aligned with the canonical conversation.
-- ASCII-only migration body avoids transport encoding issues in remote SQL APIs.
...
DROP TRIGGER IF EXISTS trg_sync_campaign_lead_on_inbound_message
  ON public.sigzap_messages;

CREATE TRIGGER trg_sync_campaign_lead_on_inbound_message
AFTER INSERT ON public.sigzap_messages
FOR EACH ROW
EXECUTE FUNCTION public.sync_campaign_lead_on_inbound_message();
...
COMMENT ON FUNCTION public.sync_campaign_lead_on_inbound_message IS
  'Moves the matching active campaign lead to em_conversa when its configured chip receives a message.';
