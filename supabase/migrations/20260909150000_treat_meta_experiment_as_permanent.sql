-- 130472 é uma decisão da Meta sobre o destinatário (grupo de experimento).
-- Não é falha transitória: retentar o mesmo número só repete a rejeição.
...
BEGIN
  ...
  -- 131026 (sem WhatsApp), 131049 (rejeição definitiva), 131042
  -- (elegibilidade/pagamento da WABA) e 130472 (experimento da Meta)
  -- nunca devem voltar para a fila.
  v_retryable := coalesce(v_code, '') NOT IN ('130472', '131026', '131042', '131049')
...
-- Retira imediatamente da fila os 130472 já registrados pela retomada.
UPDATE public.campanha_leads
SET envio_status = 'failed_permanent',
    ...
    updated_at = now()
WHERE ultimo_erro_codigo = '130472'
