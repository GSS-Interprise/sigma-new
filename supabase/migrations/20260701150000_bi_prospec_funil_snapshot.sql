-- Cache ao abrir (01/07): a tela default (sem filtro) da Prospecção agora lê um
-- SNAPSHOT pré-computado (instantâneo), recomputado por cron. Só filtro chama a RPC
-- ao vivo. Também adiciona base_total (todos os leads) pra explicar o "sem especialidade".
...
-- === Snapshot do payload default (sem filtro) — cache lido pela tela ao abrir ===
create table if not exists public.bi_prospec_funil_snapshot (
  id smallint primary key default 1,
  payload jsonb not null,
  atualizado_em timestamptz not null default now(),
  constraint bi_prospec_funil_snapshot_single check (id = 1)
);
grant select on public.bi_prospec_funil_snapshot to authenticated, service_role, anon;

create or replace function public.refresh_bi_prospec_funil_snapshot() returns void language plpgsql
security definer set search_path = public as $fn$
begin
  insert into public.bi_prospec_funil_snapshot (id, payload, atualizado_em)
  values (1, public.get_bi_prospec_funil(null, null), now())
  on conflict (id) do update set payload = excluded.payload, atualizado_em = now();
end $fn$;

select public.refresh_bi_prospec_funil_snapshot();

select cron.schedule('bi-prospec-funil-snapshot-20min', '*/20 * * * *',
  $$select public.refresh_bi_prospec_funil_snapshot();$$);
