-- 1. Coluna para rastrear telefones marcados como sem WhatsApp (NoZap)
ALTER TABLE public.leads
  ADD COLUMN IF NOT EXISTS telefones_inativos text[] NOT NULL DEFAULT ARRAY[]::text[];

COMMENT ON COLUMN public.leads.telefones_inativos IS
  'Telefones (formato livre / E.164 sem +) marcados como sem WhatsApp via callback 5-NOZAP. Usado por gerar_disparo_zap para escolher próximo número.';

-- 2. Atualizar gerar_disparo_zap:
--    - Escolhe o primeiro telefone (phone_e164 -> telefones_adicionais) que NÃO esteja em telefones_inativos
--    - Considera 5-NOZAP como status final para permitir tentar próximo número em nova execução
...
BEGIN
  ...
  FOR v_lead IN
    SELECT l.id AS lead_id, l.nome, l.phone_e164, l.telefone, l.telefones_adicionais, l.telefones_inativos
  ...
  LOOP
    -- Normaliza lista de telefones inativos (apenas dígitos, com prefixo 55)
    ...
    -- Escolhe o primeiro telefone (principal -> adicionais) que NÃO esteja inativo
    v_phone_raw := NULL;
    v_phone_e164 := NULL;

    -- Tenta principal
    IF v_lead.phone_e164 IS NOT NULL OR v_lead.telefone IS NOT NULL THEN
      v_candidato := COALESCE(v_lead.phone_e164, v_lead.telefone);
    ...
    -- Se principal está inativo, tenta os adicionais
    ...
    -- Todos os telefones inativos -> ignora lead
    IF v_phone_e164 IS NULL THEN
      v_ignorados := v_ignorados + 1;
      CONTINUE;
    END IF;

    -- Já existe disparo ativo para esse lead nesta campanha_proposta?
    -- 5-NOZAP é tratado como final: permite criar nova tentativa com próximo número.
    IF EXISTS (
    ...
    -- Já existe disparo enviado/respondido com ESTE telefone? Evita duplicado
    IF EXISTS (
      SELECT 1 FROM public.disparos_contatos dc
      WHERE dc.campanha_proposta_id = p_campanha_proposta_id
        AND dc.lead_id = v_lead.lead_id
