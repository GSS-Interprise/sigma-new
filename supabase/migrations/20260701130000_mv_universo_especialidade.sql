-- Perf real (01/07): o universo por especialidade era count(distinct) sobre a
-- junção (541k) a cada chamada (~6s). Materializo em views pequenas (~3k linhas),
-- refresh diário via cron. A RPC do funil passa a ler as MVs (sub-segundo).

-- universo por especialidade × UF (distinct médicos)
create materialized view if not exists public.mv_especialidade_uf_universo as
  select le.especialidade_id,
         coalesce(nullif(btrim(l.uf), ''), '??') uf,
         count(distinct le.lead_id) n
  from public.lead_especialidades le
  join public.leads l on l.id = le.lead_id
  group by 1, 2;
create unique index if not exists ux_mv_esp_uf on public.mv_especialidade_uf_universo (especialidade_id, uf);

-- universo por UF (distinct médicos com alguma especialidade)
create materialized view if not exists public.mv_uf_universo as
  select coalesce(nullif(btrim(l.uf), ''), '??') uf, count(distinct le.lead_id) n
  from public.lead_especialidades le
  join public.leads l on l.id = le.lead_id
  group by 1;
create unique index if not exists ux_mv_uf on public.mv_uf_universo (uf);

grant select on public.mv_especialidade_uf_universo, public.mv_uf_universo to authenticated, service_role, anon;

-- refresh diário 05:00 (concurrently — não trava leitura)
select cron.schedule('mv-universo-especialidade-daily', '0 5 * * *',
  $$refresh materialized view concurrently public.mv_especialidade_uf_universo;
    refresh materialized view concurrently public.mv_uf_universo;$$);

-- RPC lê as MVs pro universo; funil/coorte/contatados seguem ao vivo (tabelas pequenas)
...
-- universo por dimensão via MV (rápido)
op_universo as (
  select (case when p_especialidade_id is null then especialidade_id::text else uf end) k,
         sum(n) universo
