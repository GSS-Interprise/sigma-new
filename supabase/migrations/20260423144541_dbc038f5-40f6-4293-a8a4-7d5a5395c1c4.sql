-- Índices para acelerar agregações
CREATE INDEX IF NOT EXISTS idx_disparos_contatos_status_data_envio ON public.disparos_contatos(status, data_envio);
CREATE INDEX IF NOT EXISTS idx_disparos_contatos_lead_id ON public.disparos_contatos(lead_id);
CREATE INDEX IF NOT EXISTS idx_disparo_manual_envios_created_lead ON public.disparo_manual_envios(created_at, lead_id);
CREATE INDEX IF NOT EXISTS idx_email_interacoes_created_direcao ON public.email_interacoes(created_at, direcao);
CREATE INDEX IF NOT EXISTS idx_cplc_created_canal ON public.campanha_proposta_lead_canais(created_at, canal);
CREATE INDEX IF NOT EXISTS idx_leads_data_conversao ON public.leads(data_conversao) WHERE data_conversao IS NOT NULL;

-- RPC agregada
CREATE OR REPLACE FUNCTION public.get_bi_prospec_dashboard(
...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- Permissão: admin OU permissão captacao.view
  ...
  WITH
  -- ========= Disparos em massa (somente efetivamente enviados) =========
  massa AS (
  ...
  -- ========= Top campanhas (vw_campanha_metricas no período) =========
  top_camp AS (
    SELECT *
...
GRANT EXECUTE ON FUNCTION public.get_bi_prospec_dashboard(timestamptz, timestamptz) TO authenticated;