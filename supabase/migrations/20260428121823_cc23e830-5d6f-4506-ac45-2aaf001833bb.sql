...
BEGIN
  ...
  manuais_chat_all AS (
    SELECT
      ...
      m.conversation_id,
      m.sent_at,
      LAG(m.sent_at) OVER (PARTITION BY m.conversation_id ORDER BY m.sent_at) AS prev_user_msg_at
  ...
  manuais_chat AS (
    SELECT lead_id, sent_at AS created_at
    FROM manuais_chat_all
    WHERE sent_at >= p_inicio AND sent_at <= p_fim
      AND (
        prev_user_msg_at IS NULL
        OR sent_at - prev_user_msg_at >= interval '7 days'
      )
  ),
  manuais AS (
  ...
  -- Resolve a especialidade canonica de cada lead com prioridade:
  -- 1) Junction lead_especialidades -> nome canonico em especialidades (1a por created_at)
  -- 2) Match por alias / nome usando texto livre leads.especialidade (case-insensitive)
  -- 3) Texto livre original (preserva dado nao mapeado)
  -- 4) 'Sem especialidade'
  lead_esp_junction AS (
    ...
    WHERE le.lead_id IN (SELECT lead_id FROM lead_ids WHERE lead_id IS NOT NULL)
      AND e.ativo = true
    ORDER BY le.lead_id, le.created_at ASC NULLS LAST
  ),
  lead_esp_alias AS (
    SELECT
      l.id AS lead_id,
    ...
    JOIN public.especialidades e
      ON e.ativo = true
     AND (
       LOWER(TRIM(l.especialidade)) = LOWER(e.nome)
       OR LOWER(TRIM(l.especialidade)) = ANY (
         SELECT LOWER(a) FROM unnest(COALESCE(e.aliases, ARRAY[]::text[])) a
       )
     )
    WHERE l.id IN (SELECT lead_id FROM lead_ids WHERE lead_id IS NOT NULL)
      ...
      AND l.id NOT IN (SELECT lead_id FROM lead_esp_junction)
  ),
  lead_esp_alias_dedup AS (
    SELECT DISTINCT ON (lead_id) lead_id, especialidade
    FROM lead_esp_alias
    ORDER BY lead_id, especialidade
  ),
  lead_esp AS (
    SELECT
      l.id,
      COALESCE(
        lj.especialidade,
        la.especialidade,
        NULLIF(TRIM(l.especialidade), ''),
        'Sem especialidade'
      ) AS especialidade
    FROM public.leads l
    LEFT JOIN lead_esp_junction   lj ON lj.lead_id = l.id
    LEFT JOIN lead_esp_alias_dedup la ON la.lead_id = l.id
    WHERE l.id IN (SELECT lead_id FROM lead_ids WHERE lead_id IS NOT NULL)
  ),
  responderam AS (
