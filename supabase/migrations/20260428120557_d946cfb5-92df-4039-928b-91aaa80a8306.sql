...
BEGIN
  ...
  -- Manuais explicitos (botao Disparo Manual)
  manuais_dme AS (
    SELECT lead_id, created_at
    FROM public.disparo_manual_envios
    WHERE created_at >= p_inicio AND created_at <= p_fim
  ),
  -- Manuais via chat SIG Zap: 1 disparo = 1 ABERTURA de conversa pelo usuario.
  -- LAG calculado sobre TODO o historico para detectar corretamente a 1a msg
  -- e reaberturas (gap >= 7 dias entre mensagens do usuario na mesma conversa).
  manuais_chat_all AS (
    SELECT
      c.lead_id,
      m.conversation_id,
